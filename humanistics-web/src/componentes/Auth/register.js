...
import { Row, Col, Form, Input, Button, Switch, Spin, Modal, Image } from 'antd';
    ...
    handleSubmit = values => {
        ...
        axios.post('/register', {
            ...
            .catch((error) => {
                ...
                Modal.warning({
                    title: 'Error',
                    content: (error.response == undefined) ? "Error al crear registro, verifique información ingresada."
                        : Array.isArray(error.response.data.message) ? error.response.data.message.map(e => <p>{e.message}</p>) : error.response.data.message
                });
            })
            .finally(f => {
    ...
    **/
    renderRedirect = () => {
        if (this.state.redirect)
            return <Redirect to="/register/steps" />
    }
    render() {
        return (
            <div className="register bg-white">
                {this.renderRedirect()}
                <Row style={{ height: "98%" }} glutter={[16, 16]}>
                                ...
                                <Col span={24} className="login-form">
                                    <Row justify="center">
                                        <Col xs={{ span: 24, offset: 0 }} sm={{ span: 21, offset: 0 }} md={{ span: 20, offset: 0 }} lg={{ span: 11, offset: 0 }} xl={{ span: 12, offset: 0 }} xxl={{ span: 12, offset: 0 }} className="mt-10">
                                            <h3 className="login-form-title">Registro</h3>
                                            <Form initialValues={{ remember: true }} onFinish={this.handleSubmit} layout={"vertical"} >
                                                <Form.Item name="nombre" label="Nombre" rules={[{ required: true, message: 'Por favor ingresa tu  nombre' }]}>
                                                    <Input prefix={<UserOutlined />} placeholder="Nombre" className="input-box" />
                                                </Form.Item>
                                                ...
                                                <Form.Item name="telefono" label="Telefono" rules={[{ required: true, message: 'Por favor ingresa tu telefono' }]}  >
                                                    <Input prefix={<PhoneOutlined />} type="number" placeholder="66412345678" className="input-box" />
                                                </Form.Item>
                                                ...
                                                <Form.Item name="accep_terms"
                                                    label="Acepto los términos y condiciones"
                                                    rules={[{
                                                        required: true,
                                                        message: 'Para continuar, por favor acepta los terminos y condiciones'
                                                    }]}>
                                                    <Switch />
                                                </Form.Item>

                                                <Form.Item>
                                                    <label style={{ marginRight: "5px", fontSize: "12pt", fontWeight: "600" }}> ¿Ya tienes cuenta? </label>

                                                    
                                                    
                                                    <Link className="login-form-link" to="/login"> Iniciar sesión </Link>
                                                </Form.Item>

                                                <Form.Item style={{ marginBottom: "0px" }}>
                                                    <Button type="submit" htmlType="submit" className="btn btn-secondary " style={{ fontWeight: "600" }} >REGISTRARME</Button>
                                                </Form.Item>
                                            </Form>
                                        </Col>
                                    </Row>
