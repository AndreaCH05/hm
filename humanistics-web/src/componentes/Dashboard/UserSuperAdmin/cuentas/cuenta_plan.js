...
class Plan extends Component {
    ...
    **/
    ...
    componentDidUpdate = async () => { 
        if (this.props.proyecto_id != "" && this.state.proyecto_id == "" ||  (this.props.proyecto_id != this.state.proyecto_id ) ) {
            this.setState({ proyecto_id: this.props.proyecto_id });
   ...
   **/
    LoadData = async (user) => {
        axios.post('/checkout/id',
            {
                usuarios_id: sessionStorage.getItem('userId'),
            ...
            .then((response) => {
                if(response.data.success) {
                    ...
                    this.setState({
                        ...
                        proyecto: response.data.data.proyecto_id
                    })
    ...
    render() {
        let { pago, usuario, plan, loading, proyecto, proyecto_nombre } = this.state;
        ...
        return (
            ...
            <Layout className="bg-white">
                <Content className="pd-1" >
                    <Row>
                        <h3 className="font_color up" > Cuentas</h3>
                    </Row>
                    <div className="container-shadow pd-2" style={{ height: "100vh" }}>

                        <Row>
                            <Col span={24}><h3 className="font_color up" style={{ width: 'calc(100% - 50px)', color: '#a0a7ab' }}> Plan  {proyecto_nombre}</h3></Col>
                            ...
                            <Col span={8} className="center"><p className="cuenta-plan-info">Prospectos del Mes <span className="cuenta-plan-info-content">{plan.prospectos_mensuales}</span></p> </Col>
...
class PlanesList extends Component {
    ...
    InfoProyecto = async () => {
        axios.post('/dashboard',
            { proyecto_id: this.state.proyecto_id })
            .then((response) => {
                this.state.proyecto_nombre = response.data.proyecto.nombre;
    ...
    render() {
        return (
            <Layout className="bg-white pd-1">
                <Row>
                    <Col span={24} ><p className="modal-title subtitle-section center">Plan de Prospectos  {this.state.proyecto_nombre}</p></Col>
